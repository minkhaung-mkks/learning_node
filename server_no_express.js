...
const server = http.createServer((req, res) => {
    ...
    myEmitter.emit('log', `${req.method}: ${req.url}`, 'Req_logs.log')
    let fileLocale;
    let fileExtentsion;
    let contentType;
    // Good Way
    ...
    const servreFile = async (filePath, contentType, res) => {
        try {
            const rawData = await fsPromise.readFile(filePath, !contentType.includes('image') ? 'utf-8' : '');
            const data = contentType === 'application/json' ? JSON.parse(rawData) : rawData
            res.writeHead(
                filePath.includes('404') ? 404 : 200, { 'Content-Type': contentType })
            ...
            console.error(error);
            myEmitter.emit('log', `${error.name}: ${error.message}`, 'Server_error.log')
            res.statusCode = 500;
            res.end();
        }
    }
